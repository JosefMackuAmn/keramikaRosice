...
const generateHeader = (invoice, order) => {
  ...
  const invoiceNumber =
    "KR-" +
    order.variableSymbol.slice(0, -4) +
    "/" +
    order.variableSymbol.slice(-4);
...
const generatePersonInformation = (invoice, order) => {
  ...
  invoice
    ...
    .text(
      `${helpers.replaceHtmlEntities(order.city)} ${order.zipCode}`,
      306,
      170
    )
...
const generateInvoiceData = (invoice, order) => {
  const [deliveryLabel, paymentLabel] = helpers.getDelPayLabel(
    order.delivery,
    order.payment
  );
...
const generateProductsTable = (invoice, order) => {
  ...
  order.items.forEach((item) => {
    invoice
      ...
      .text(`${item.product.price * item.amount}Kč`, 0, 390 + deltaY, {
        align: "right",
      })
...
module.exports = (order, invoicePath) => {
  ...
  invoice.registerFont(
    "PTSans-Regular",
    path.join(__dirname, "/../public/fonts/PTSans-Regular.ttf")
  );
  invoice.registerFont(
    "PTSans-Bold",
    path.join(__dirname, "/../public/fonts/PTSans-Bold.ttf")
  );
